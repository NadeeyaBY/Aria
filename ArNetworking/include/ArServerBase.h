...
public:
  ...
  AREXPORT void remData(const char *name);
protected:
...
//  XXX todo remove template <typename T> void packAndSend(ArServerClient *client, ArNetPacket *pkt, T* ptr);
//  XXX todo remove template <typename T> void packAndSend(ArServerClient *client, ArNetPacket *pkt, ArRetFunctor<T> accessorFunc);
};
