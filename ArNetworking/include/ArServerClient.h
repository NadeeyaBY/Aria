...
public:
  ...
  **/
  ...
  /// Get creation time
  ArTime getCreationTime(void) { return myCreationTime; }

  /// Find name of command for the given number. Return NULL if no command with the given ID is known.
  AREXPORT const char *findCommandName(unsigned int command) const;
