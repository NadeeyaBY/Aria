...
public:
  /// Constructor
  ArClientData(const char *name, const char *description, 
  ...
  int tryLockFunctorList(void) { return myMutex.tryLock(); }
  /// Unlocks the functor list so we can walk it without it changing
  int unlockFunctorList(void) { return myMutex.unlock(); }
  
  /// Adds a new functor the end of the list
  void addFunctor(ArFunctor1<ArNetPacket *> *functor, bool ownFunctor = false) 
    { myFunctorList.push_back(functor); 
      if(ownFunctor) myOwnFunctors.insert(functor);
    }
  /// Removes a functor from the list all together
  /// If we own the functor, delete it (using delete)
  void remFunctor(ArFunctor1<ArNetPacket *> *functor) 
    { myFunctorList.remove(functor); 
      std::set< ArFunctor1<ArNetPacket*> *>::iterator f = myOwnFunctors.find(functor);
      if(f != myOwnFunctors.end())
      {
        delete (*f);
        myOwnFunctors.erase(f);
      }
    }
...
protected:
  ...
  std::set<ArFunctor1<ArNetPacket *> *> myOwnFunctors;
};
