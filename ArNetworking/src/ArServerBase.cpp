...
AREXPORT void ArServerBase::close(void)
{
  ...
  {
    client = (*it);
    ArLog::log(ArLog::Verbose, "%sSending shutdown to client %d/%s/%s", myLogPrefix.c_str(), client->getIdentifier().getConnectionID(), client->getIdentifier().getIDString(), client->getIPString());
    client->shutdown();
 ...
 * does not return in a timely manner data reception will be delayed (blocked).
   ...
   @param argumentDescription a specification of the arguments expected, see
    below for syntax

   @param returnDescription a specification of what the data returns, see below
    for syntax
   ...
   @a argumentDescription and @a returnDescription are small specifications of
   the format of the arguments (if any) given in the request packet and the format of the
   reply packet (if any).  

   Note, not all data requests follow the syntax below exactly, but most do.

   They must have the following syntax:
  
   <blockquote>
   [<b>&lt;repeat...&gt;</b>|<b>listN:</b>] <i>datatype</i><b>:</b> [<i>multiplier</i> <b>*</b>] <i>description</i>[<b>,|;</b> <i>datatype</i><b>:</b> <i>description</i>...] [<b>}</b>]
   </blockquote>

   Where <i>datatype</i> is one of:
   <blockquote>
   byte
   byte2
   byte4
   ...
   string
   list&lt;<i>datatype</i>&gt;
   map&lt;<i>datatype, datatype</i>&gt;
   </blockquote>
   Interpretation of these names should be case-insensitive.

   byte, byte2, byte4, ubyte, ubyte2 and ubyte4 types refer to corresponding 
   variable sizes in ArTypes.   double and float refer to 4-byte signed integers that
   have been shifted by multiplication by some power of 10.  string refers to a 
   NULL-terminated string.   Some byte2, byte4, ubyte2 and ubyte4 values have
   been similarly converted but the multiplier is given in the description, e.g.  
   "byte2: 10 * value" indicates that the value was multiplied by 10 before
   being added to the packet.

   For most straightforward fields the description may just consist of a
   one-word name.  Others may have a longer description that incldues spaces.
   
   Multiple fields may be listed wtih differing types, separated by either
   commas or semicolons. If the tag &lt;repeat ...&lt; or &lt;repeating ...&lt; appears before a field, then
   the field may be repeated (whether the number of items in this list is given
   elsewhere in the packet or a special terminating value ends the list, or the 
   list simply runs to the end of the packet, is specific to the individual data
   request or commands.)  The text following the word "repeat" or "repeating" is sometimes
   an explanation of this.
  
   Or for simple arrays/lists or maps, use list<> or map<>.

   Any text in <i>description</i> contained within parentheses is not parsed
(i.e. any commas or semicolons inside the parentheses should be ignored).
   

   Some commands or data requests have unique behavior that can be described more loosely in 
   @a argumentDescription and @a returnDescription, but any automated parsing
   may fail requiring manual implementations specific to these unique requests or
   commands.
...
AREXPORT void ArServerBase::remData(const char *name)
{
  // XXX TODO
}
