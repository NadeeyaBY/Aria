...
/**
   ...
   @param takeOwnershipOfFunctor True if ArClientBase should own this functor,
    including deleting it if this handler is removed with remHandler() and the
    functor is no longer needed. Use this if you use new to allocate the functor,
    pass it in, but don't keep any reference to it for later deallocation.
               ...
               bool takeOwnershipOfFunctor)
{
  ...
  // addthe functor
  clientData->addFunctor(functor, takeOwnershipOfFunctor);
  clientData->unlockFunctorList();
  return true;
}
...
AREXPORT bool ArClientBase::remHandler(const char *name, 
				       ArFunctor1<ArNetPacket *> *functor)
{
  ArLog::log(myVerboseLogLevel, "%s: Removing handler for \"%s\"...", myLogPrefix.c_str(), name);
  ArClientData *clientData;
  myMapsMutex.lock();
